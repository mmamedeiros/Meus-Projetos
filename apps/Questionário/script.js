...
btnRestart.onclick = () =>{
    content.computedStyleMap.display = "flex";
    contentFinish.computedStyleMap.display = "none";
...
function nextQuestion(e){
    if(e.target.getAttribute("data-correct") === "true"){
        questionsCorrect ++; // verifica se você acertou a questãoi  e caso tenha acertado ele atribui um ponto
    }
...
function finish(){{
    ...
    contentFinish.computedStyleMap.display = "flex";
}}
...
function loadQuestion(){ // funçaõ rresponsavel por rodar as questões
    ...
    item.ansewrs.forEach((answer) =>{
        const div = document.createElement("div"); // cria um elemento div no html

        div.innerHTML = `
        <button class="answer" data-current="${answer.correct}">
